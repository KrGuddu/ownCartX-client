...
import { deleteReview, postReview } from "../../store/slices/productSlice"; //importate
      ...
      {authUser && (
        <form onSubmit={handleReviewSubmit} className="mb-8 space-y-4">
          ...
          <button
            ...
            className="px-6 py-2 rounded-lg bg-primary text-primary-foreground font-semibold hover:glow-on-hover animate-smooth disabled:opacity-50" //submit review text color change => text-primary-foreground
          >
                      ...
                      <div className="flex">
                        {[...Array(5)].map((_, i) => {
                          return (
                            <Star
                              key={i}
                              className={`w-4 h-4 ${i < Math.floor(review.rating) ? "text-yellow-400 fill-current" : "text-gray-300"}`} //check here rating or ratings
                            />
                          );
                        })}
                      </div>
                        ...
                        onClick={() =>
                          dispatch(
                            deleteReview({
                              ...
                              reviewId: review.review_id,   //check reviewId or review_id
                            }),
                          )
                        }
