...
export const forgotPassword = createAsyncThunk(
  ...
  async (email, thunkAPI) => {
    try {
      // const res = await axiosInstance.post("/auth/password/forgot?frontendUrl=http://localhost:5173", email);      // { email }
      const res = await axiosInstance.post(`/auth/password/forgot?frontendUrl=${window.location.origin}`, email);      // { email }
      toast.success(res.data.message);
      return null;
    } catch (error) {
      toast.error(error.response.data.message);
